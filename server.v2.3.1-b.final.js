...
/* =========================
   CORS estricto por ALLOWED_ORIGINS (JSON array)
   ========================= */
const ORIGINS = (() => {
...
/* =========================
   PUBLIC_BASE (acepta PUBLIC_BASE o PUBLIC_BASE_URL)
   ========================= */
...
/* =========================
   Helpers
   ========================= */
// Normaliza nombres de archivo (evita caracteres problemáticos en keys S3)
const SAFE_CHARS_RE = /[\\#?<>:*|"\\x00-\\x1F]/g;
const safeName = (name) => name.replace(SAFE_CHARS_RE, '_');
...
async function headObject(Key) {
  try { return await s3.headObject({ Bucket: BUCKET, Key }).promise(); }
...
/* =========================
   Health / raíz (para Render)
   ========================= */
app.get('/', (_req, res) => {
  res.type('text/plain').send('Mixtli Transfer backend v2.3.1-b OK. Usa /api/health');
});
...
/* =========================
   Crear transferencia (multi-archivo, link único)
   ========================= */
app.post('/api/transfers', upload.array('files', 50), async (req, res) => {
  try {
    ...
    const link = backendBase + viewPath;                    // siempre disponible
    const publicLink = PUBLIC_BASE ? `${PUBLIC_BASE}${viewPath}` : link; // dominio bonito si existe

    return res.json({ id, link, publicLink, expiresInDays, count: items.length, totalBytes: total });
  } catch (err) {
    console.error(err);
...
/* =========================
   Obtener manifest (aplica expiración)
   ========================= */
app.get('/api/transfers/:id', async (req, res) => {
  try {
    ...
    if (new Date(manifest.expiresAt) < new Date()) return res.status(410).json({ error: 'expired' });
    return res.json(manifest);
  } catch (err) {
    return res.status(500).json({ error: err.message || 'get_manifest_failed' });
  }
});
...
/* =========================
   Descargar todo como ZIP (streaming)
   ========================= */
app.get('/api/transfers/:id/download.zip', async (req, res) => {
  try {
    ...
    for (const f of manifest.files) {
      ...
      const objHead = await headObject(key);
      if (!objHead) continue; // skip si falta
      const stream = await getObjectStream(key);
...
<div class="footer">Mixtli Transfer v2.3.1-b — compat: PUBLIC_BASE / PUBLIC_BASE_URL.</div>
</div></div></body></html>`;
...
/* =========================
   ...
   ========================= */
app.listen(PORT, () => {
  console.log('Mixtli Transfer backend v2.3.1-b listening on', PORT);
});
